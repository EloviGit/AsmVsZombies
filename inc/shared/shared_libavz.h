#ifndef __SHARED_LIBAVZ_H__
#define __SHARED_LIBAVZ_H__

#include "avz_multi_platform.h"

#include FILE_SHARED(avz_asm.h)
#include FILE_SHARED(avz_card.h)
#include FILE_SHARED(avz_click.h)
#include FILE_SHARED(avz_cob_manager.h)
#include FILE_SHARED(avz_connector.h)
#include FILE_SHARED(avz_coroutine.h)
#include FILE_SHARED(avz_exception.h)
#include FILE_SHARED(avz_global.h)
#include FILE_SHARED(avz_iterator.h)
#include FILE_SHARED(avz_logger.h)
#include FILE_SHARED(avz_memory.h)
#include FILE_SHARED(avz_painter.h)
#include FILE_SHARED(avz_pvz_struct.h)
#include FILE_SHARED(avz_seh.h)
#include FILE_SHARED(avz_smart.h)
#include FILE_SHARED(avz_state_hook.h)
#include FILE_SHARED(avz_tick_runner.h)
#include FILE_SHARED(avz_timeline.h)
#include FILE_SHARED(avz_time_queue.h)
#include FILE_SHARED(avz_types.h)

#endif
